...
import { AjouterspecificComponent } from '../ajouterspecific/ajouterspecific.component';
import { AjouterspeciapplicationComponent } from '../ajouterspeciapplication/ajouterspeciapplication.component';
import { DeleteapplicationComponent } from '../deleteapplication/deleteapplication.component';
import { ConsultService } from 'src/app/shared/servicess/services/consult.service';
@Component({
  selector: 'app-dashboardteg',
  templateUrl: './dashboardteg.component.html',
  styleUrls: ['./dashboardteg.component.css']
})
export class DashboardtegComponent implements OnInit {
  headerajouter: string = 'AJOUTER FONCTION';
  headerajouterapplication: string = 'AJOUTER APPLICATION';
...
selectChangememe1() {
  ...
  searchfonction(info) {
    ...
    this.api.getsingleFonction(info.fnom)
      .subscribe(res => {
  ...
  searchapplication(infoapplication) {
    console.log("infoapplication is",infoapplication)
    infoapplication.appNom=this.val2;
    this.api.getsingleApplication(infoapplication.appNom)
      .subscribe(res => {
  ...
  addBlack() {
    this.getAllEmployee();
    const ref = this.dialogService.open(AjouterspecificComponent, {
      width: 'auto',
      closeOnEscape: false,
  ...
  addApplicat() {
    this.getAllEmployee();
    const ref = this.dialogService.open(AjouterspeciapplicationComponent, {
      width: 'auto',
      closeOnEscape: false,
  ...
  onDeletematapplica(obj) {
    let data = obj;
    const ref = this.dialogService.open(DeleteapplicationComponent, {
      closeOnEscape: false,
      data: { obj: data }
    });
    ref.onClose.subscribe(res => {
